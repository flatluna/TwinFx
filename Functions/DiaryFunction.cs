...
namespace TwinFx.Functions
{
    ...
    public class DiaryFunction
    {
        ...
        public DiaryFunction(ILogger<DiaryFunction> logger, DiaryCosmosDbService diaryService, IConfiguration configuration, DiaryAgent diaryAgent)
        {
            _logger = logger;
                        ...
                        if (!string.IsNullOrEmpty(diaryEntry.PathFile))
                        {
                            ...
                            if (!string.IsNullOrEmpty(sasUrl) && IsImageFile(diaryEntry.PathFile))
                            {
                                ...
                                if (extractionResult.Success && extractionResult.ExtractedData != null)
                                {
                                    ...
                                    _logger.LogInformation("🎯 Llamando a GenerateComprehensiveAnalysisAsync con datos reales...");
                                    ...
                                    {
                                        _logger.LogInformation("✅ ===== ANÁLISIS COMPRENSIVO COMPLETADO CON RECIBO ===== Tiempo: {ProcessingTime}ms", 
                                            comprehensiveAnalysis.ProcessingTimeMs);
                                        _logger.LogInformation("📋 Executive Summary preview: {Summary}", 
                                            comprehensiveAnalysis.ExecutiveSummary.Length > 200 
                                                ? comprehensiveAnalysis.ExecutiveSummary.Substring(0, 200) + "..." 
                                                : comprehensiveAnalysis.ExecutiveSummary);
                                        _logger.LogInformation("📊 HTML Report length: {HtmlLength} characters", 
                                            comprehensiveAnalysis.DetailedHtmlReport.Length);
                                        _logger.LogInformation("🔍 Metadata count: {MetadataCount} items", 
                                            comprehensiveAnalysis.Metadata.Count);
                                    }
                                    else
                                    {
                                        _logger.LogWarning("⚠️ ===== ANÁLISIS COMPRENSIVO CON RECIBO FALLÓ ===== Error: {ErrorMessage}", 
                                            comprehensiveAnalysis.ErrorMessage);
                                    }
                                }
                                else
                                {
                                    _logger.LogWarning("⚠️ Receipt extraction failed, fallback a análisis sin recibo");
                                    await ExecuteAnalysisWithoutReceipt(diaryEntry);
                                }
                            }
                            else
                            {
                                _logger.LogInformation("📄 Archivo no es imagen o SAS URL no generado, fallback a análisis sin recibo. SasUrl: {HasSasUrl}, IsImage: {IsImage}", 
                    ...
                    try
                    {
                        if (IsImageFile(originalFileName))
                        {
                            ...
                            if (extractionResult.Success && extractionResult.ExtractedData != null)
                            {
                                ...
                                // 🧠 NUEVO: Generar análisis comprensivo combinando diario y recibo
                                // Para este caso (ProcessFileUploadForCreate) no tenemos el diaryEntry disponible 
                                // ya que se está creando. El análisis comprensivo se hace desde CreateDiaryEntry
        ...
        private async Task<string> ExtractDiaryDataFromMultipart(HttpRequestData req)
        {
            try
            {
                var contentTypeHeader = req.Headers.FirstOrDefault(h => h.Key.Equals("Content-Type", StringComparison.OrdinalIgnoreCase));
                ...
                // Buscar la parte que contiene 'diaryData'
                var diaryDataPart = parts.FirstOrDefault(p => p.Name == "diaryData");
                
                if (diaryDataPart != null && !string.IsNullOrEmpty(diaryDataPart.StringValue))
                {
                    _logger.LogInformation("📄 Found diaryData field in multipart: {Length} chars", diaryDataPart.StringValue.Length);
                    return diaryDataPart.StringValue;
                }
                
                _logger.LogInformation("📄 No diaryData field found in multipart, using fallback parsing");
                return null;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "❌ Error extracting diary data from multipart");
                return null;
            }
        }
            ...
            string twinId, string entryId, DiaryEntry? diaryEntry = null)
        {
                ...
                if (uploadSuccess)
                {
                    ...
                    try
                    {
                        if (IsImageFile(originalFileName))
                        {
                           ...
                           var Result_Analcis = await _diaryAgent.GenerateComprehensiveAnalysisAsync(extractionResult, diaryEntry);
            ...
            {
                _logger.LogError(ex, "❌ Error uploading file from part");
                return string.Empty;
            }
        }
        ...
        /// <summary>
        /// Determine activity type from file name for better extraction
        /// </summary>
        ...
        /// <summary>
        /// Execute comprehensive analysis without receipt data (for testing or entries without files)
        /// </summary>
        private async Task ExecuteAnalysisWithoutReceipt(DiaryEntry diaryEntry)
        {
            try
            {
                ...
                if (comprehensiveAnalysis.Success)
                {
                    ...
                    _logger.LogInformation("📊 HTML Report length: {HtmlLength} characters", 
                        comprehensiveAnalysis.DetailedHtmlReport.Length);
                    _logger.LogInformation("🔍 Metadata keys: {MetadataKeys}", 
                        string.Join(", ", comprehensiveAnalysis.Metadata.Keys));
                    
                    // TODO: En el futuro, aquí podrías guardar el análisis en la base de datos
                    // o devolverlo en la respuesta al cliente
                }
                else
                {
