...
public class WorkFunctions
{
    ...
    )
    {
        ...
        try
        {
            ...
            // Create specialized Work Documents Cosmos DB service
            var workDocumentsService = new WorkDocumentsCosmosService(
            ...
            var workDocuments = await workDocumentsService.GetWorkDocumentsByTwinIdAsync(twinId);
            ...
            try
            {
                ...
                foreach (var doc in workDocuments)
                {
                    try
                    {
                        ...
                        try
                        {
                            // FIX: Get containerName from the document dictionary, default to twinId
                            var containerName = doc.GetValueOrDefault("containerName")?.ToString() ?? 
                                               doc.GetValueOrDefault("TwinID")?.ToString() ?? 
                                               twinId;
                            
                            var dataLakeClient = dataLakeFactory.CreateClient(containerName);
                            var filePath = doc.GetValueOrDefault("filePath")?.ToString() ?? "";
                            var fileName = doc.GetValueOrDefault("fileName")?.ToString() ?? "";
                            ...
                            if (!string.IsNullOrEmpty(filePath) && !string.IsNullOrEmpty(fileName))
                            {
                                ...
                                {
                                    // Add SAS URL to the document dictionary
                        ...
                        {
                            _logger.LogWarning(ex, "⚠️ Could not generate SAS URL for work document {Id}", doc.GetValueOrDefault("id"));
                        }

                    }
                    catch (Exception innerEx)
                    {
                        _logger.LogWarning(innerEx, "⚠️ Error while ensuring SAS URL for work document {Id}", doc?.GetValueOrDefault("id"));
                    }
                }
            }
            catch (Exception ex)
    ...
    private async Task<FormattedResumeData> FormatResumeForUIAsync(Dictionary<string, object?> resumeDoc, DataLakeClientFactory dataLakeFactory, string twinId)
    {
        try
        {
            ...
            {
                // FIX: Get containerName from the document dictionary, default to twinId
                var containerName = resumeDoc.GetValueOrDefault("containerName")?.ToString() ?? 
                                   resumeDoc.GetValueOrDefault("TwinID")?.ToString() ?? 
                                   twinId;
    ...
    private async Task<WorkDocumentData> FormatWorkDocumentForUIAsync(Dictionary<string, object?> workDoc, DataLakeClientFactory dataLakeFactory, string twinId)
    {
        try
        {
            ...
            {
                // FIX: Get containerName from the document dictionary, default to twinId
                var containerName = workDoc.GetValueOrDefault("containerName")?.ToString() ?? 
                                   workDoc.GetValueOrDefault("TwinID")?.ToString() ?? 
                                   twinId;
            ...
            return new WorkDocumentData
            {
                ...
                Stats = new TwinFx.Models.WorkDocumentStats
                {
